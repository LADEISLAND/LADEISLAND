...
export default function Register() {
  ...
  const [leaderRole, setLeaderRole] = useState('president')
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await api.post('/auth/register', { username, password, leader_role: leaderRole })
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <form onSubmit={onSubmit} className="border rounded p-6 w-full max-w-sm space-y-3">
        ...
        <select className="w-full border rounded px-3 py-2" value={leaderRole} onChange={e => setLeaderRole(e.target.value)}>
